          ...
          {Array.from({ length: COPIAS }, (_, copia) =>
            CONQUISTAS.map((c, r) => {
              ...
              return (
                ...
                >
                  <motion.figure
                    className="relative aspect-square overflow-hidden rounded-[1.5rem] bg-grafite sm:aspect-[4/3] md:rounded-[2rem] lg:aspect-[16/10]"
                    initial={false}
                    animate={{
                      scale: ativo ? 1 : 0.88,
                      filter: ativo ? 'brightness(1) saturate(1)' : 'brightness(0.5) saturate(0.6)',
                    }}
                  ...
                  >
                    <motion.img
                      ...
                      initial={false}
                      animate={ativo && !reduzir ? { scale: [1.12, 1] } : { scale: 1.04 }}
                      transition={ativo ? { duration: DURACAO + 1.2, ease: [0.2, 0.6, 0.35, 1] } : { duration: 1, ease: EASE }}
                    />
                    <motion.div
                      aria-hidden="true"
                      className="absolute inset-x-0 bottom-0 h-3/5 bg-linear-to-t from-black/80 via-black/35 to-transparent"
                      initial={false}
                    ...
                    <figcaption className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-6 p-5 md:p-8 lg:p-10">
                      <span className="block max-w-[16ch] overflow-hidden font-display text-[clamp(1.55rem,3.2vw,3rem)] leading-[1.04] tracking-[-0.018em] text-branco">
                        <motion.span
                          className="block"
                          initial={false}
                          animate={{ y: ativo ? '0%' : '130%' }}
                          transition={{ duration: 0.95, ease: EASE, delay: ativo ? 0.35 : 0 }}
                        >
                          {c.legenda}
                        </motion.span>
                      </span>
                      <motion.span
                        className="shrink-0 font-display text-[0.95rem] tabular-nums text-prata"
                        initial={false}
                        animate={{ opacity: ativo ? 1 : 0 }}
                        transition={{ duration: 0.6, delay: ativo ? 0.5 : 0 }}
                      >
