        ...
        <div ref={palco} className={reduzir ? 'relative min-h-[100svh]' : 'sticky top-0 h-[100svh] overflow-hidden'}>
          <div className="absolute inset-0 flex flex-col items-center pt-[max(5.5rem,11svh)] lg:flex-row lg:items-center lg:justify-end lg:pr-[7vw] lg:pt-0">
            <div ref={mostrador} className="relative aspect-square w-[min(80vw,44svh)] lg:w-[min(44vw,74svh)]">
              <motion.div className="absolute inset-0" style={reduzir ? undefined : { scale: dialEscala, opacity: dialOpacidade }}>
          ...
          <motion.div
            className="absolute inset-x-0 bottom-0 pb-[max(3rem,env(safe-area-inset-bottom))] lg:inset-y-0 lg:right-auto lg:flex lg:w-[52%] lg:items-center lg:pb-0"
          ...
          {!reduzir && <motion.div aria-hidden="true" className="absolute inset-0 bg-nevoa" style={{ clipPath: inundacao }} />}
        </div>
      </div>
    </Folha>
  )
...
function Mostrador({ progresso, estatico }: { progresso: MotionValue<number>; estatico: boolean }) {
  ...
  return (
    <svg viewBox="0 0 600 600" className="absolute inset-0 h-full w-full" role="img" aria-label="Segredo de cofre com o monograma DBC">
      <defs>
      ...
      <motion.g style={girar(girarAnel)}>
        <circle cx={C} cy={C} r={280} fill="none" stroke="none" />
        {marcasAro}
      </motion.g>
      <circle cx={C} cy={C} r={248} fill="none" stroke="rgba(255,255,255,0.07)" />

      {/* trincos que recolhem quando o segredo fecha */}
      ...
      {/* o segredo numerado, girando */}
      <motion.g style={girar(girarSegredo)}>
      ...
      {/* placa central fixa com o monograma */}
