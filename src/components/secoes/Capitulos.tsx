...
function Camada({ capitulo, i, p }: { capitulo: Capitulo; i: number; p: MotionValue<number> }) {
  ...
  return (
    <div className="absolute inset-0" style={{ zIndex: i + 1 }}>
      <motion.div className="absolute inset-0 overflow-hidden will-change-transform" style={{ clipPath: recorte }}>
        <motion.img
        ...
        />
        <motion.div aria-hidden="true" className="absolute inset-0 bg-preto will-change-[opacity]" style={{ opacity: sombra }} />
      </motion.div>
      <motion.span
        aria-hidden="true"
        className="absolute inset-x-0 top-1/2 h-px bg-branco shadow-[0_0_16px_2px_rgba(255,255,255,0.6)] will-change-transform"
